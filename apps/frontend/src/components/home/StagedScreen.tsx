...
export default function StagedScreen({ onOpenChannel }: StagedScreenProps) {
  ...
  return (
    <div className="my-auto grid grid-cols-1 items-center gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:gap-16">
      <div className="min-w-0">
        ...
        <div className="mt-8">
          {files.map((f, i) => (
            <div
              key={f.id}
              className="flex min-w-0 flex-col gap-2 border-b border-border-subtle py-4.5 sm:flex-row sm:items-baseline sm:gap-4.5"
            >
              <div className="flex min-w-0 items-baseline gap-4.5 sm:flex-1">
                ...
                <span className="min-w-0 flex-1 wrap-break-word text-lg font-medium">
                  {f.name}
                </span>
              </div>

              <div className="flex shrink-0 items-center justify-end gap-4.5">
      ...
      <div className="min-w-0 rounded-lg bg-surface px-7 py-7.5">
